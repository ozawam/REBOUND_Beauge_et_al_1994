...
//	if (p1.lastcollision==t || p2.lastcollision==t) return;
	...
	double eps= 0.7; // perfect bouncing by default //change in 2019/05/18
	...
	double vr1,vr2;
	vr1 = particles[c.p1].vx * cos(phi1) + particles[c.p1].vy * sin(phi1);
        vr2 = particles[c.p2].vx * cos(phi2) + particles[c.p2].vy * sin(phi2);
       ...
       r1 = pow(3.0*particles[c.p1].m/(4.0*M_PI*rho_planet),1.0/3.0);
       r2 = pow(3.0*particles[c.p2].m/(4.0*M_PI*rho_planet),1.0/3.0);
       vesc12 = sqrt(2.0*r->G*(particles[c.p1].m + particles[c.p2].m)/(r1 + r2));
