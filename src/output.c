...
void reb_output_timing(struct reb_simulation* r, const double tmax){
    ...
    printf("walltime = %- 9f [m] , %- 9f [h] ",r->walltime/60.0,r->walltime/60.0/60.0);
#ifdef PROFILING
...
//MAX and MIN
double maxElement(struct reb_simulation* r, size_t size){
    size_t i;
    double max;

    max = r->particles[1].m;
    for (i = 1; i < size; ++i) {
        if (max < r->particles[i].m) {
            max = r->particles[i].m;
        }
    }
...
double minElement(struct reb_simulation* r, size_t size){
    size_t i;
    double min;

    min = r->particles[1].m;
    for (i = 1; i < size; ++i) {
        if (min > r->particles[i].m) {
            min = r->particles[i].m;
        }
    }
...
//mean eccentricity of field bodies
double meaneccentricity(struct reb_simulation* r,size_t size,double M_unit,double min){
    double mean_e;
    double mean_inc;
    double sigma_e = 0.0;
    double sigma_inc = 0.0;
    int n_field = 0;
    ...
    for(int i=1;i<size;i++){
       if((r->particles[i].m)*M_unit == min){ 
         struct reb_orbit o = reb_tools_particle_to_orbit(r->G, r->particles[i],com);
         sigma_e += pow(o.e,2.0);
         sigma_inc += pow(o.inc,2.0);              
         n_field += 1;
            }
   }
      mean_e = sqrt(sigma_e/n_field);
      mean_inc = sqrt(sigma_inc/n_field);
...
void reb_output_orbits(struct reb_simulation* r, char* filename){
    ...
    char filename_mpi[1024];
    sprintf(filename_mpi,"%s_%d",filename,r->mpi_id);
    FILE* of = fopen(filename_mpi,"w"); //2019/02/27change mode a->w 
  //  FILE* of = fopen(filename_mpi,"a"); 
#else // MPI
    FILE* of = fopen(filename,"w"); //2019/02/27change mode a->w 
  //  FILE* of = fopen(filename,"a");  
#endif // MPI
    if (of==NULL){
        reb_exit("Can not open file.");
...
//MAX and MIN
    int SIZE_OF_ARRAY = N ;
    double max = maxElement(r, SIZE_OF_ARRAY)*M_unit;
    double min = minElement(r, SIZE_OF_ARRAY)*M_unit;

//mean eccentricity of field bodies
   double mean_e = meaneccentricity(r,SIZE_OF_ARRAY,M_unit,min);

//central star mass
 //  double star_mass = r->particles[0].m * M_sun ; 
 // Star
    struct reb_particle star = {0};
    star.m = 1;
    star.r = 0.1;

    fprintf(of,"%d\t%e\t%e\t%e\t%e\t%e\t\n",N,(r->t)*(T_unit/one_year),max,min,mean_e,star.m);

    struct reb_particle com = r->particles[0];
    for (int i=1;i<N;i++){
        struct reb_orbit o = reb_tools_particle_to_orbit(r->G, r->particles[i], r->particles[0]);//2019/03/21 change com -> r->particles[0] -> star
        double _rho_planet = 2.0;//g/cm^3
        double rho_planet = _rho_planet * 1.49597871e13 * 1.49597871e13 * 1.49597871e13 / 1.9884e33; // [Msun/AU^3]
	double m_50 = 4.0/3.0 * pow(5000.0/one_au,3.0) *M_PI * rho_planet;
        double rate_50 = 8.958600e+25/M_sun/m_50;
	double rplanet = cbrt(3.0*r->particles[i].m/rate_50/(4.0*M_PI*rho_planet)) * one_au/100.0;//m
	//      fprintf(of,"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n",r->t,o.a,o.e,o.inc,o.Omega,o.omega,o.l,o.P,o.f);
        fprintf(of,"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e[m]\n",(r->t)*(T_unit/one_year),(r->particles[i].m)*M_unit,o.a,o.e,o.inc,o.Omega,o.omega,o.l,o.P,o.f,rplanet);   //2019/02/26 edit, add mass output, t year, m g, a AU 2019/0319 add N  
        com = reb_get_com_of_pair(com,r->particles[i]); //2019/02/27 change
    }
    fclose(of);
}

void reb_output_orbits_WH(struct reb_simulation* r, char* filename){
    const int N = r->N;
#ifdef MPI
    char filename_mpi[1024];
    sprintf(filename_mpi,"%s_%d",filename,r->mpi_id);
...
//Unit is AU, Msun and day
    double M_sun=1.989*pow(10.0,33.0);//g
    double one_au=1.49597870*pow(10.0,13.0);//cm
    double one_year = 365.0;//year-day    
...
//central star mass
 ...
 // Star
    ...
    fprintf(of,"%d\t%e\t%e\t%e\t%e\t%e\t\n",N,(r->t)/one_year,max,min,mean_e,star.m);

    struct reb_particle com = r->particles[0];
    for (int i=1;i<N;i++){
        struct reb_orbit o = reb_tools_particle_to_orbit(r->G, r->particles[i], r->particles[0]);//2019/03/21 change com -> r->particles[0] -> star
//      fprintf(of,"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n",r->t,o.a,o.e,o.inc,o.Omega,o.omega,o.l,o.P,o.f);
        fprintf(of,"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n",(r->t)/one_year,(r->particles[i].m)*M_unit,o.a,o.e,o.inc,o.Omega,o.omega,o.l,o.P,o.f);   //2019/02/26 edit, add mass output, t year, m g, a AU 2019/0319 add N  
        com = reb_get_com_of_pair(com,r->particles[i]); //2019/02/27 change
    }
    fclose(of);
}
